...
onPaste(event: ClipboardEvent) {
  if(event.clipboardData){
    ...
    this.imgView = event.clipboardData.getData('text')
...
setImg(e: any){
  ...
  this.imgView =e.target.value
}

async pasteImg(){
 
  const copiedData = await navigator.clipboard.readText()
  this.formulario.get('img')?.setValue(copiedData)
  this.imgView = copiedData
